...
static Key keys[] = {
	...
	{ MODKEY|ShiftMask,             XK_m,      spawn,          SHCMD("man -k . | awk '{print $1}' | dmenu -sb '#33333f' -sf '#3355ff' | xargs -r man -Tpdf > /tmp/manual.pdf && emacsclient -c /tmp/manual.pdf && rm -f /tmp/manual.pdf") },
